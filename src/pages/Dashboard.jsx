...
export default function Dashboard({ onNavigate, onFilterOS }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: osAbertas } = await supabase.from('ordens_servico')
        .select('id,numero_ordem,titulo,prioridade,data_abertura,executado_por,areas(nome),equipamentos(nome,codigo),mecanicos(nome),status_os!inner(nome,cor,cor_bg,icone)')
      ...
      let recQ = supabase.from('ordens_servico').select('id,numero_ordem,titulo,prioridade,data_abertura,status_os(nome,cor,cor_bg,icone),areas(nome)').order('data_abertura',{ascending:false}).limit(15)
  ...
  const OSList = ({ items, title, emptyMsg }) => (
    <div style={S.card}>
      <h3 style={{ margin:'0 0 10px',fontSize:11,color:'#64748B',textTransform:'uppercase',letterSpacing:1 }}>{title} ({items.length})</h3>
      {items.length===0?<div style={{color:'#94A3B8',fontSize:12,padding:12,textAlign:'center'}}>{emptyMsg}</div>:
        <div style={{maxHeight:280,overflow:'auto'}}>{items.map(o=>
          <div key={o.id} style={{display:'flex',alignItems:'center',gap:6,padding:'6px 0',borderBottom:'1px solid #F1F5F9',flexWrap:'wrap',cursor:'pointer'}} onClick={()=>onNavigate?.('ordens')}>
            <span style={{flex:1,fontSize:12,color:'#334155',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',minWidth:60}}>{'#'+(o.numero_ordem||'') + ' ' + (o.titulo||'').substring(0,40)}</span>
            <span style={{fontSize:10,color:'#64748B'}}>{o.areas?.nome||''}</span>
            {o.equipamentos?.nome&&<span style={{fontSize:10,color:ACCENT}}>{o.equipamentos.nome}</span>}
            <PrioBadge p={o.prioridade}/>
            <span style={{fontSize:9,color:'#94A3B8'}}>{fmtDate(o.data_abertura)}</span>
          </div>
        )}</div>}
    </div>
  )
  ...
  return <div>
    ...
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16,flexWrap:'wrap',gap:10}}>
      <h1 style={{margin:0,fontWeight:800,fontSize:vp.isMobile?22:30,letterSpacing:2,color:ACCENT}}>DASHBOARD</h1>
      <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}>
        {PRESETS.map(p=><button key={p.label} onClick={()=>applyPreset(p)} style={{
          ...
          color:preset===p.label?ACCENT:'#64748B',
          borderColor:preset===p.label?ACCENT:'#CBD5E1',
        }}>{p.label}</button>)}
        <button onClick={()=>setCustom(!custom)} style={{...S.btnS,padding:'6px 12px',minHeight:30,fontSize:10,color:custom?ACCENT:'#64748B',borderColor:custom?ACCENT:'#CBD5E1'}}>📅 Personalizado</button>
      </div>
    </div>
    {custom&&<div style={{display:'flex',gap:8,marginBottom:12,alignItems:'center',flexWrap:'wrap'}}>
      ...
      <span style={{color:'#94A3B8',fontSize:11}}>até</span>
    ...
    <div style={{display:'flex',gap:vp.isMobile?8:12,flexWrap:'wrap',marginBottom:20}}>
      ...
      <KPI label={dateFilter.from?'No Período':'Total OS'} value={dateFilter.from?kpis.periodTotal:kpis.totalOS} accent="#64748B" small={vp.isMobile}/>
    </div>
    ...
    {mecAtendendo.length>0&&<div style={{...S.card,borderLeft:'3px solid #3B82F6',marginBottom:16}}>
      ...
      {mecAtendendo.map((m,i)=><div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid #F1F5F9',flexWrap:'wrap',gap:6}}>
        <div>
          ...
          <div style={{fontSize:12,color:'#334155'}}>{(m.os||'').substring(0,60)}</div>
        </div>
    ...
    <div style={{display:'grid',gridTemplateColumns:vp.isMobile?'1fr':'1fr 1fr',gap:14}}>
      <div style={S.card}>
        <h3 style={{margin:'0 0 10px',fontSize:11,color:'#64748B',textTransform:'uppercase',letterSpacing:1}}>OS por Área {dateFilter.from?'(período)':'(ativas)'}</h3>
        {osPorArea.length===0?<div style={{color:'#94A3B8',fontSize:12,padding:12,textAlign:'center'}}>Sem dados</div>:
          osPorArea.slice(0,12).map(([a,c])=>{const mx=Math.max(...osPorArea.map(x=>x[1]));return<div key={a} style={{marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',fontSize:11,marginBottom:3}}><span style={{color:'#334155'}}>{a}</span><span style={{color:ACCENT,fontWeight:700}}>{c}</span></div>
            <div style={{background:'#F1F5F9',borderRadius:3,height:5}}><div style={{background:`linear-gradient(90deg,${ACCENT},#F59E0B)`,height:'100%',borderRadius:3,width:`${(c/mx)*100}%`}}/></div>
          </div>})}
      </div>

      <div style={S.card}>
        <h3 style={{margin:'0 0 10px',fontSize:11,color:'#64748B',textTransform:'uppercase',letterSpacing:1}}>⚠️ Estoque Baixo ({kpis.pecasBaixo})</h3>
        {pecasBaixo.length===0?<div style={{color:'#22C55E',fontSize:12,padding:12,textAlign:'center'}}>Estoque OK</div>:
          pecasBaixo.slice(0,8).map(p=><div key={p.id} style={{display:'flex',justifyContent:'space-between',padding:'4px 0',borderBottom:'1px solid #F1F5F9',fontSize:11}}>
            <span style={{color:'#334155'}}>{p.nome}</span>
      ...
      <div style={S.card}>
        <h3 style={{margin:'0 0 10px',fontSize:11,color:'#64748B',textTransform:'uppercase',letterSpacing:1}}>OS Recentes</h3>
        {osRecentes.map(o=><div key={o.id} style={{display:'flex',alignItems:'center',gap:6,padding:'5px 0',borderBottom:'1px solid #F1F5F9',flexWrap:'wrap'}}>
          <span style={{flex:1,fontSize:11,color:'#334155',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{'#'+(o.numero_ordem||'') + ' ' + (o.titulo||'').substring(0,30)}</span>
