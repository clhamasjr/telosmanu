  ...
  return <div>
    ...
    <Modal open={!!modal} onClose={() => setModal(null)} title={modal === 'novo' ? 'NOVO PLANO PREVENTIVO' : modal === 'exportar' ? 'EXPORTAR PLANO' : 'EDITAR PLANO'} mobile={vp.isMobile}>
      {item && modal !== 'exportar' && <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16, paddingTop: 14, borderTop: '1px solid #1F1F23', gap: 8, flexWrap: 'wrap' }}>
...
function PrevMateriais({ planoId }) {
  ...
  return <div style={{ background: '#1A1A1E', borderRadius: 8, padding: 12, marginTop: 14 }}>
    <div style={{ fontSize: 10, color: '#888', textTransform: 'uppercase', fontWeight: 600, marginBottom: 8 }}>📦 Materiais Necessários ({mats.length})</div>
    {mats.map(m => <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid #222', fontSize: 11 }}>
      <span style={{ color: '#CCC' }}>{m.materiais?.nome || m.descricao} ({m.materiais?.codigo})</span>
...
function ExportarPlano({ plano, equipamentos, areas, onDone, onCancel }) {
  ...
  return <div>
    <div style={{ background: '#1A1A1E', borderRadius: 8, padding: 12, marginBottom: 14, borderLeft: '3px solid #A855F7' }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: '#A855F7' }}>Exportando: {plano.descricao}</div>
      <div style={{ fontSize: 11, color: '#888', marginTop: 4 }}>Selecione os equipamentos que receberão cópia deste plano.</div>
    </div>
    ...
    <div style={{ maxHeight: 250, overflow: 'auto', border: '1px solid #222', borderRadius: 6 }}>
      {filtered.map(eq => <label key={eq.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 10px', cursor: 'pointer', borderBottom: '1px solid #1A1A1E', background: selected.includes(eq.id) ? '#A855F710' : 'transparent' }}>
        <input type="checkbox" checked={selected.includes(eq.id)} onChange={() => toggle(eq.id)} style={{ width: 16, height: 16 }} />
        <span style={{ fontSize: 12, color: selected.includes(eq.id) ? '#A855F7' : '#CCC' }}>{eq.nome}{eq.codigo ? ` (${eq.codigo})` : ''}</span>
      </label>)}
    </div>
    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16, paddingTop: 14, borderTop: '1px solid #1F1F23' }}>
