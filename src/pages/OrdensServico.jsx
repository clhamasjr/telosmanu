...
export default function OrdensServico({ initialStatusFilter, onClearFilter }) {
  ...
  // Load description catalog
  useEffect(() => {
    supabase.from('descricoes_padrao').select('*').eq('ativo', true).order('categoria').order('descricao')
      .then(({ data }) => setDescPadrao(data || []))
  }, [])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      while (true) {
        ...
        if (cancelled || !rows || rows.length === 0) break
        all = all.concat(rows)
        if (rows.length < 500) break
        pg++; if (pg > 40) break
      }
      if (!cancelled) { setOrdens(all); setLoading(false) }
    })()
  ...
  return <div>
    ...
    <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
      {[['mes', 'Mês Atual'], ['anterior', 'Mês Anterior'], ['trimestre', 'Trimestre'], ['tudo', 'Tudo']].map(([k, l]) =>
        <button key={k} onClick={() => aplicarPreset(k)} style={{
          ...
          background: preset === k ? ACCENT + '22' : 'transparent', color: preset === k ? ACCENT : '#888', borderColor: preset === k ? ACCENT : '#2A2A30',
        }}>{l}</button>
      )}
    ...
    <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
      ...
      <span style={{ fontSize: 10, color: '#555' }}>{filtered.length} OS{loading ? '...' : ''}</span>
    </div>
      ...
      vp.isMobile ? <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{filtered.slice(0, 100).map(o =>
        <div key={o.id} style={{ ...S.card, padding: 12 }} onClick={() => { setOs({ ...o }); setModal('ver') }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: ACCENT }}>OS #{o.numero_ordem || '—'}</span>
      ...
      <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr>
        ...
        <tr key={o.id} style={{ cursor: 'pointer' }} onMouseEnter={e => e.currentTarget.style.background = '#1A1A1E'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'} onClick={() => { setOs({ ...o }); setModal('ver') }}>
          ...
          <td style={S.td}><div style={{ display: 'flex', gap: 3 }}>
            {!isSolic && <button style={{ ...S.btnS, padding: '3px 6px', minHeight: 26, fontSize: 9 }} onClick={e => { e.stopPropagation(); setOs({ ...o }); setModal('editar') }}>✏️</button>}
            {isMec && o.status_os?.nome !== 'Concluída' && <button style={{ ...S.btnS, padding: '3px 6px', minHeight: 26, fontSize: 9, color: '#3B82F6', borderColor: '#3B82F6' }} onClick={e => { e.stopPropagation(); setOs({ ...o }); setModal('atender') }}>🔧</button>}
            {isGestor && o.status_os?.nome === 'Aguardando Aprovação' && <button style={{ ...S.btnS, padding: '3px 6px', minHeight: 26, fontSize: 9, color: '#22C55E', borderColor: '#22C55E' }} onClick={e => { e.stopPropagation(); setOs({ ...o }); setModal('aprovar') }}>✅</button>}
          </div></td>
        </tr>
      )}</tbody></table>
        {filtered.length > 200 && <div style={{ textAlign: 'center', padding: 6, color: '#555', fontSize: 10 }}>200 de {filtered.length}</div>}
      </div>
    }
    ...
    <Modal open={modal === 'nova' || modal === 'editar'} onClose={() => setModal(null)} title={modal === 'nova' ? 'NOVA ORDEM DE SERVIÇO' : 'EDITAR OS #' + (os?.numero_ordem || '')} mobile={vp.isMobile}>
      {os && <OSForm os={os} setOs={setOs} onSave={salvar} onCancel={() => setModal(null)}
        onDel={modal === 'editar' && isGestor ? () => setConfirm({ msg: 'Excluir?', ok: async () => { await supabase.from('ordens_servico').delete().eq('id', os.id); setConfirm(null); setModal(null); refetch() } }) : null}
        ...
        saving={saving} mobile={vp.isMobile} perfil={perfil} />}
    </Modal>
    ...
    <Modal open={modal === 'ver'} onClose={() => setModal(null)} title={'ORDEM DE SERVIÇO Nº ' + (os?.numero_ordem || '—')} mobile={vp.isMobile}>
      {os && <OSDetail os={os} onEdit={() => setModal(isMec ? 'atender' : 'editar')} onAprovar={() => setModal('aprovar')} mobile={vp.isMobile} perfil={perfil} statusList={statusList} refetch={refetch} />}
    </Modal>
...
// ── Formulário ──
function OSForm({ os, setOs, onSave, onCancel, onDel, areas, equipamentos, mecanicos, statusList, tiposMan, tiposFalha, descPadrao, isEdit, saving, mobile, perfil }) {
  ...
  if (!os.tipo_manutencao_id) missing.push('Tipo')
  if (!os.area_id) missing.push('Área')
  const canSave = missing.length === 0

  // Group descriptions by category
  const descByCat = useMemo(() => {
    const map = {}
    descPadrao.forEach(d => { const c = d.categoria || 'Geral'; if (!map[c]) map[c] = []; map[c].push(d) })
    return map
  }, [descPadrao])
  ...
  return <div>
    ...
    <div style={{ background: '#1A1A1E', borderRadius: 8, padding: 14, marginBottom: 14, borderLeft: '3px solid ' + ACCENT }}>
      <div style={{ display: 'flex', gap: 14, marginBottom: 10, alignItems: 'center' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 9, color: '#666', textTransform: 'uppercase' }}>Equipamento</div>
          <select style={{ ...S.select, fontSize: 14, padding: '10px 12px', fontWeight: 600 }} value={os.equipamento_id || ''} onChange={e => u('equipamento_id', e.target.value)}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr', gap: '0 14px' }}>
      <Field label="Solicitante" req>
        <input style={{ ...S.input, background: '#1A1A1E', fontWeight: 600 }} value={os.solicitante || ''} readOnly={isSolic} onChange={e => u('solicitante', e.target.value)} />
      </Field>
    ...
    <Field label="Descrição do Problema / Serviço" req>
      {descPadrao.length > 0 && <select style={{ ...S.select, marginBottom: 6, fontSize: 11, color: '#888' }} value="" onChange={e => { if (e.target.value) u('descricao', (os.descricao ? os.descricao + '. ' : '') + e.target.value) }}>
        ...
        {Object.entries(descByCat).map(([cat, items]) =>
          <optgroup key={cat} label={cat}>{items.map(d => <option key={d.id} value={d.descricao}>{d.descricao}</option>)}</optgroup>
        )}
      </select>}
      <textarea style={{ ...S.input, minHeight: 80, resize: 'vertical' }} value={os.descricao || ''} onChange={e => u('descricao', e.target.value)} placeholder="Descreva o problema ou selecione acima" />
    </Field>
...
// ── Atender (mecânico) → status vai para Aguardando Aprovação ──
function AtenderOS({ os, statusList, onDone, mobile }) {
  ...
  const salvar = async () => {
    setSaving(true)
    // Mecânico finaliza → Aguardando Aprovação
    const stAprov = statusList.find(s => s.nome === 'Aguardando Aprovação')
    const stAnd = statusList.find(s => s.nome === 'Em Andamento')
    const upd = {
      descricao_execucao: descExec,
      status_id: stAprov?.id || os.status_id,
      data_conclusao: new Date().toISOString(),
    }
    if (!os.data_inicio) upd.data_inicio = os.data_abertura
    await supabase.from('ordens_servico').update(upd).eq('id', os.id)
    setSaving(false); onDone()
  }

  const iniciar = async () => {
    const stAnd = statusList.find(s => s.nome === 'Em Andamento')
    await supabase.from('ordens_servico').update({ status_id: stAnd?.id, data_inicio: new Date().toISOString() }).eq('id', os.id)
    onDone()
  }
  ...
  return <div>
    ...
    {isAberta && <div style={{ textAlign: 'center', marginBottom: 14 }}>
      ...
      <div style={{ fontSize: 10, color: '#666', marginTop: 6 }}>Registra data/hora de início automaticamente</div>
    </div>}
    ...
    {!isAberta && <>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, paddingTop: 12, borderTop: '1px solid #1F1F23' }}>
        <button style={{ ...S.btnP, background: '#22C55E' }} onClick={salvar} disabled={saving}>{saving ? 'Salvando...' : '✅ Finalizar (Enviar p/ Aprovação)'}</button>
      </div>
    </>}
  </div>
}

// ── Aprovar OS (gestor/solicitante) ──
function AprovarOS({ os, statusList, onDone }) {
  const [saving, setSaving] = useState(false)
  const aprovar = async () => {
    setSaving(true)
    const stConc = statusList.find(s => s.nome === 'Concluída')
    await supabase.from('ordens_servico').update({ status_id: stConc?.id }).eq('id', os.id)
    setSaving(false); onDone()
  }
  const rejeitar = async () => {
    setSaving(true)
    const stAnd = statusList.find(s => s.nome === 'Em Andamento')
    await supabase.from('ordens_servico').update({ status_id: stAnd?.id, data_conclusao: null }).eq('id', os.id)
    setSaving(false); onDone()
  }
  return <div>
    <div style={{ background: '#1A1A1E', borderRadius: 8, padding: 14, marginBottom: 16 }}>
      ...
      {os.descricao_execucao && <div style={{ fontSize: 12, color: '#93C5FD', marginTop: 8, lineHeight: 1.5, background: '#0D2137', padding: 10, borderRadius: 6 }}><strong>Serviço realizado:</strong> {os.descricao_execucao}</div>}
    </div>
    ...
    <div style={{ fontSize: 10, color: '#666', textAlign: 'center', marginTop: 8 }}>Aprovar encerra a OS. Devolver retorna ao mecânico.</div>
  </div>
}

// ── Detalhes SOFMAN ──
function OSDetail({ os, onEdit, onAprovar, mobile, perfil, statusList, refetch }) {
  ...
  const isGestor = perfil === 'gestor' || perfil === 'admin'
  const isSolic = perfil === 'solicitante'
  const canAprovar = isAprov && (isGestor || (isSolic && os.solicitante))

  const R = ({ l, v, a }) => <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #1A1A1E' }}>
    <span style={{ fontSize: 10, color: '#666', textTransform: 'uppercase', fontWeight: 600 }}>{l}</span>
    <span style={{ fontSize: 12, color: a || '#CCC', fontWeight: a ? 700 : 400 }}>{v || '—'}</span>
  </div>
  ...
  return <div>
    <div style={{ background: '#1A1A1E', borderRadius: 8, padding: 16, marginBottom: 14, border: '1px solid #2A2A30' }}>
      ...
      {os.equipamentos && <div style={{ marginBottom: 6 }}>
        <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase' }}>Equipamento</div>
    ...
    <R l="Solicitante" v={os.solicitante} a={ACCENT} />
    <R l="Data Abertura" v={fmtDT(os.data_abertura)} />
    ...
    {os.mecanicos && <R l="Mecânico" v={os.mecanicos.nome} />}

    {os.descricao && <div style={{ marginTop: 10 }}>
    ...
    {mats.length > 0 && <div style={{ marginTop: 12 }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr><th style={{ ...S.th, fontSize: 9, padding: '4px 6px' }}>Descrição</th><th style={{ ...S.th, fontSize: 9, padding: '4px 6px' }}>Un.</th><th style={{ ...S.th, fontSize: 9, padding: '4px 6px' }}>Qtde</th></tr></thead>
        <tbody>{mats.map(m => <tr key={m.id}><td style={{ ...S.td, fontSize: 11, padding: '4px 6px' }}>{m.materiais?.nome || m.descricao}</td><td style={{ ...S.td, fontSize: 11, padding: '4px 6px', color: '#888' }}>{m.materiais?.unidade}</td><td style={{ ...S.td, fontSize: 11, padding: '4px 6px', color: '#3B82F6', fontWeight: 700 }}>{m.quantidade}</td></tr>)}</tbody></table>
    </div>}

    {/* Approval banner */}
    {canAprovar && <div style={{ marginTop: 14, background: '#422006', border: '1px solid #F59E0B', borderRadius: 8, padding: 14, textAlign: 'center' }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: '#F59E0B', marginBottom: 8 }}>⏳ Aguardando sua aprovação</div>
    ...
    <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: 14, marginTop: 12, borderTop: '1px solid #1F1F23', gap: 8 }}>
      {isSolic && !canAprovar ? <span style={{ fontSize: 11, color: '#666' }}>Acompanhe o status acima</span> :
        !canAprovar && <button style={S.btnP} onClick={onEdit}>{perfil === 'mecanico' ? '🔧 Atender' : '✏️ Editar'}</button>}
    </div>
  </div>
}
