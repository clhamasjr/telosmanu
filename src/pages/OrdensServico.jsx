...
export default function OrdensServico() {
  ...
  return <div>
    ...
    {filtered.length===0 ? <Empty icon="📋" msg="Nenhuma OS" action="Criar OS" onAction={novaOS}/> :
      vp.isMobile ? <div style={{display:'flex',flexDirection:'column',gap:12}}>{filtered.slice(0,100).map(o=>
        <div key={o.id} style={S.card} onClick={()=>{setOs({...o});setModal('ver')}}>
          ...
          <div style={{fontSize:14,fontWeight:600,color:'#E5E5E5',marginBottom:6,lineHeight:1.3}}>{(o.titulo||'Sem título').substring(0,80)}</div>
          <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}><StatusBadge status={o.status_os}/><span style={{fontSize:12,color:'#888'}}>{o.areas?.nome||''}</span><span style={{fontSize:10,color:'#555'}}>{fmtDate(o.data_abertura)}</span></div>
        </div>
      )}</div> :
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>
        <th style={S.th}>Nº</th><th style={S.th}>Título</th><th style={S.th}>Área</th><th style={S.th}>Equip.</th><th style={S.th}>Status</th><th style={S.th}>Prioridade</th><th style={S.th}>Data</th><th style={S.th}></th>
      ...
      {filtered.length > 200 && <div style={{textAlign:'center',padding:12,color:'#666',fontSize:12}}>Mostrando 200 de {filtered.length}. Use os filtros para refinar.</div>}
      </div>
    }
...
function OSForm({ os,setOs,onSave,onCancel,onDel,areas,equipamentos,mecanicos,statusList,tiposMan,tiposFalha,isEdit,saving,mobile }) {
  ...
  // Filter equipment by area if area is selected, but ALWAYS show all as fallback
  const filteredEquip = useMemo(() => {
    if (os.area_id) {
      const areaEquip = equipamentos.filter(e => e.area_id === os.area_id)
      if (areaEquip.length > 0) return areaEquip
    }
    // Show ALL equipment if no area selected or area has no equipment
    return equipamentos
  ...
  return <div>
    <div style={{display:'grid',gridTemplateColumns:g,gap:'0 16px'}}>
      ...
      <Field label={`Equipamento (${filteredEquip.length} disponíveis)`}><select style={S.select} value={os.equipamento_id||''} onChange={e=>u('equipamento_id',e.target.value)}>
        <option value="">Nenhum</option>
        {filteredEquip.map(e=><option key={e.id} value={e.id}>{e.nome}{e.codigo?` (${e.codigo})`:''}{e.tag?` [${e.tag}]`:''}</option>)}
      </select></Field>
...
function OSDetail({ os, onEdit, mobile }) {
  ...
  return <div>
    ...
    {os.descricao&&<div style={{marginBottom:16}}><div style={{fontSize:10,color:'#666',textTransform:'uppercase',marginBottom:6}}>Descrição</div><div style={{fontSize:13,color:'#CCC',lineHeight:1.6,background:'#1A1A1E',padding:12,borderRadius:8,maxHeight:200,overflow:'auto'}}>{os.descricao}</div></div>}
