  ...
  return <div>
      ...
      {filtered.map(e=>{
        ...
        return <div key={e.id} style={{...S.card,borderLeft:'3px solid '+sc}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
            <span style={{fontWeight:700,color:'#0F172A',fontSize:14}}>{e.nome}</span>
    ...
    <Modal open={!!modal} onClose={()=>setModal(null)} title={modal==='novo'?'NOVO EQUIPAMENTO':'EDITAR EQUIPAMENTO'} mobile={vp.isMobile}>
      {item&&<div>
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginTop:20,paddingTop:16,borderTop:'1px solid #E2E8F0',gap:10,flexWrap:'wrap'}}>
...
function EquipHistorico({ equip }) {
    ...
    (async () => {
      ...
      while (true) {
        const { data: rows } = await supabase.from('ordens_servico')
          .select('id,numero_ordem,titulo,data_abertura,data_conclusao,data_inicio,tempo_execucao_min,tempo_atendimento_min,executado_por,status_os(nome,cor,icone),tipos_manutencao(nome),tipos_falha(nome)')
  ...
  if (data.loading) return <div style={{textAlign:'center',padding:30,color:'#94A3B8'}}>Carregando...</div>
  ...
  return <div>
    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginBottom:14}}>
      {[['Total OS',totalOS,ACCENT],['Paradas',paradas,'#EF4444'],['Concluídas',concl,'#22C55E'],['T.Médio',`${tMed.toFixed(0)}min`,'#3B82F6']].map(([l,v,c])=>
        <div key={l} style={{background:'#F1F5F9',borderRadius:6,padding:8,textAlign:'center'}}>
          <div style={{fontSize:20,fontWeight:900,color:c}}>{v}</div>
          <div style={{fontSize:8,color:'#94A3B8',textTransform:'uppercase'}}>{l}</div>
        </div>)}
    </div>
    <div style={{marginBottom:14}}>
      <div style={{fontSize:10,color:'#64748B',textTransform:'uppercase',marginBottom:6,fontWeight:600}}>Motivos Recorrentes</div>
      {falhas.length===0?<div style={{color:'#94A3B8',fontSize:11}}>Sem dados</div>:
        falhas.slice(0,6).map(([f,c])=>{const mx=Math.max(...falhas.map(x=>x[1]));return<div key={f} style={{marginBottom:5}}>
          <div style={{display:'flex',justifyContent:'space-between',fontSize:11,marginBottom:2}}><span style={{color:'#334155'}}>{f}</span><span style={{color:ACCENT,fontWeight:700}}>{c}x</span></div>
          <div style={{background:'#F1F5F9',borderRadius:3,height:4}}><div style={{background:'linear-gradient(90deg,#EF4444,#F59E0B)',height:'100%',borderRadius:3,width:`${(c/mx)*100}%`}}/></div>
        </div>})}
    </div>
    {Object.keys(anoMap).length>0&&<div style={{marginBottom:14}}>
      <div style={{fontSize:10,color:'#64748B',textTransform:'uppercase',marginBottom:6,fontWeight:600}}>OS por Ano</div>
      <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>{Object.entries(anoMap).sort().map(([y,c])=>
        <div key={y} style={{background:'#F1F5F9',borderRadius:6,padding:'4px 10px',textAlign:'center'}}><div style={{fontSize:14,fontWeight:700,color:ACCENT}}>{c}</div><div style={{fontSize:8,color:'#94A3B8'}}>{y}</div></div>
      )}</div>
    </div>}
    {data.pecas.length>0&&<div style={{marginBottom:14}}>
      <div style={{fontSize:10,color:'#64748B',textTransform:'uppercase',marginBottom:6,fontWeight:600}}>Peças Vinculadas ({data.pecas.length})</div>
      {data.pecas.map(p=><div key={p.id} style={{display:'flex',justifyContent:'space-between',padding:'3px 0',borderBottom:'1px solid #F1F5F9',fontSize:11}}>
        <span style={{color:'#334155'}}>{p.materiais?.nome} ({p.materiais?.codigo})</span>
        <span style={{color:'#64748B'}}>Estoque: <strong style={{color:(p.materiais?.quantidade||0)<=0?'#EF4444':ACCENT}}>{p.materiais?.quantidade||0}</strong></span>
      </div>)}
    </div>}
    <div>
      <div style={{fontSize:10,color:'#64748B',textTransform:'uppercase',marginBottom:6,fontWeight:600}}>Últimas OS ({Math.min(totalOS,30)})</div>
      <div style={{maxHeight:220,overflow:'auto'}}>{data.os.slice(0,30).map(o=><div key={o.id} style={{padding:'5px 0',borderBottom:'1px solid #F1F5F9',fontSize:11}}>
        <div style={{display:'flex',justifyContent:'space-between'}}><span style={{color:'#0F172A',fontWeight:600}}><span style={{color:ACCENT}}>#{o.numero_ordem||'—'}</span> {(o.titulo||'').substring(0,40)}</span><span style={{color:'#94A3B8'}}>{fmtDate(o.data_abertura)}</span></div>
        <div style={{color:'#64748B'}}>{o.status_os?.icone} {o.status_os?.nome}{o.tipos_falha?.nome?' · '+o.tipos_falha.nome:''}{o.tempo_execucao_min>0?' · '+o.tempo_execucao_min+'min':''}</div>
      </div>)}</div>
    </div>
  </div>
}
...
export function Areas() {
  ...
  // Color map for areas
  const areaColors = {
    'Linha A':'#2563EB','Linha B':'#7C3AED','Linha C':'#0EA5E9','Linha D':'#D97706','Linha E':'#DC2626',
    'Acabamento':'#16A34A','Branqueamento':'#EC4899','Abridor':'#F59E0B','Preparação':'#6366F1',
    'Prensa':'#EA580C','Caldeira':'#DC2626','Silo':'#84CC16','Fábrica':'#64748B',
    'Manutenção':'#0F172A','Administrativo':'#475569','Almoxarifado':'#A855F7',
  }

  return <div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20,flexWrap:'wrap',gap:10}}>
      <h1 style={{margin:0,fontSize:vp.isMobile?22:28,fontWeight:800,color:'#0F172A',letterSpacing:-.5}}>ÁREAS & EQUIPAMENTOS</h1>
      <button style={S.btnP} onClick={()=>{setItem({nome:'',descricao:'',responsavel:''});setModal('novo')}}>+ Área</button>
    </div>
      ...
      {data.map(a=>{
        ...
        return <div key={a.id} style={{...S.card,cursor:'pointer'}} onClick={()=>setSelArea(selArea===a.id?null:a.id)}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            <div>
              <div style={{fontWeight:800,fontSize:20,letterSpacing:1,color:ACCENT,marginBottom:4}}>{a.nome}</div>
              <div style={{fontSize:12,color:'#64748B',marginBottom:8}}>{a.descricao||'Sem descrição'}</div>
            </div>
          ...
          <div style={{display:'flex',gap:12,fontSize:12}}>
            ...
            <span style={{color:'#64748B'}}>Total: {eqs.length}</span>
          </div>
          {a.responsavel&&<div style={{fontSize:11,color:'#94A3B8',marginTop:6}}>Resp.: {a.responsavel}</div>}

          {/* Lista de equipamentos expandida */}
          {selArea===a.id&&eqs.length>0&&<div style={{marginTop:12,paddingTop:12,borderTop:'1px solid #E2E8F0'}}>
            <div style={{fontSize:11,color:'#64748B',textTransform:'uppercase',marginBottom:8}}>Equipamentos desta área</div>
            {eqs.map(eq=>{
              const sc=eq.status==='Operando'?'#22C55E':eq.status==='Parado'?'#EF4444':'#F59E0B'
              return <div key={eq.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:'1px solid #F1F5F9',cursor:'pointer'}} onClick={e=>{e.stopPropagation();setHistEquipArea(eq)}}>
                <div><span style={{fontSize:13,color:'#0F172A',fontWeight:600}}>{eq.nome}</span>{eq.codigo&&<span style={{fontSize:11,color:ACCENT,marginLeft:8}}>({eq.codigo})</span>}</div>
          ...
          {selArea===a.id&&eqs.length===0&&<div style={{marginTop:12,paddingTop:12,borderTop:'1px solid #E2E8F0',fontSize:12,color:'#94A3B8'}}>Nenhum equipamento nesta área</div>}
        </div>
      })}
    </div>
    ...
    {filtered.length===0?<Empty icon="👨‍🔧" msg="Nenhum mecânico" action="Cadastrar" onAction={novo}/>:
      <div style={{display:'grid',gridTemplateColumns:vp.isMobile?'1fr':'repeat(auto-fill,minmax(340px,1fr))',gap:14}}>
        {filtered.map(m=><div key={m.id} style={{...S.card}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
            <span style={{fontWeight:700,fontSize:15,color:'#0F172A'}}>{m.nome}</span>
...
function MecDetalhe({ nome, mecanicoId, periodo, setPeriodo }) {
  const { data: osList, loading } = useOSPorMecanico(nome, periodo, mecanicoId)
  ...
  return <div>
    ...
    <div style={{display:'flex',gap:12,marginBottom:16}}>
      <div style={{background:'#F1F5F9',borderRadius:8,padding:'10px 16px',flex:1,textAlign:'center'}}>
        <div style={{fontSize:24,fontWeight:700,color:ACCENT}}>{osList.length}</div>
        <div style={{fontSize:10,color:'#94A3B8',textTransform:'uppercase'}}>OS Atendidas</div>
      </div>
      <div style={{background:'#F1F5F9',borderRadius:8,padding:'10px 16px',flex:1,textAlign:'center'}}>
        <div style={{fontSize:24,fontWeight:700,color:'#3B82F6'}}>{Math.floor(totalHoras/60)}h{String(totalHoras%60).padStart(2,'0')}</div>
        <div style={{fontSize:10,color:'#94A3B8',textTransform:'uppercase'}}>Horas Totais</div>
      </div>
    </div>
    {loading?<div style={{color:'#94A3B8',textAlign:'center',padding:20}}>Carregando...</div>:
      osList.length===0?<div style={{color:'#94A3B8',textAlign:'center',padding:20}}>Nenhuma OS no período</div>:
      <div style={{maxHeight:300,overflow:'auto'}}>{osList.map(o=>
        <div key={o.id} style={{padding:'8px 0',borderBottom:'1px solid #F1F5F9',fontSize:12}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
            <span style={{color:'#0F172A',fontWeight:600}}><span style={{color:ACCENT}}>#{o.numero_ordem||'—'}</span> {(o.titulo||'OS').substring(0,35)}</span>
            <span style={{color:'#64748B'}}>{fmtDate(o.data_abertura)}</span>
          </div>
...
export function Pecas() {
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = `estoque_manutelos_${new Date().toISOString().split('T')[0]}.csv`
  ...
  return <div>
    ...
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexWrap:'wrap',gap:10}}>
      <h1 style={{margin:0,fontWeight:800,fontSize:vp.isMobile?22:30,letterSpacing:2,color:ACCENT}}>ESTOQUE DE PEÇAS</h1>
    ...
    <div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap'}}>
      <div style={{background:'#FFFFFF',border:'1px solid #E2E8F0',borderRadius:8,padding:'8px 14px',fontSize:12}}>
        <span style={{color:'#64748B'}}>Total: </span><strong style={{color:'#0F172A'}}>{data.length}</strong>
      </div>
      <div style={{background:'#FFFFFF',border:'1px solid #E2E8F0',borderRadius:8,padding:'8px 14px',fontSize:12}}>
        <span style={{color:'#64748B'}}>Baixo: </span><strong style={{color:'#F59E0B'}}>{data.filter(p=>p.quantidade<=p.estoque_minimo&&p.quantidade>0).length}</strong>
      </div>
      <div style={{background:'#FFFFFF',border:'1px solid #E2E8F0',borderRadius:8,padding:'8px 14px',fontSize:12}}>
        <span style={{color:'#64748B'}}>Zerado: </span><strong style={{color:'#EF4444'}}>{data.filter(p=>p.quantidade===0).length}</strong>
      </div>
    </div>
    ...
    {filtered.length===0?<Empty icon="📦" msg="Nenhuma peça" action="Cadastrar" onAction={nova}/>:
      vp.isMobile?<div style={{display:'flex',flexDirection:'column',gap:12}}>{filtered.map(p=>{
        ...
        return <div key={p.id} style={{...S.card,borderLeft:'3px solid '+(zero?'#EF4444':low?'#F59E0B':'#22C55E')}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
            <span style={{fontWeight:700,color:'#0F172A',fontSize:14,cursor:'pointer',textDecoration:'underline',textDecorationColor:'#333'}} onClick={e=>{e.stopPropagation();setMatDetail(p)}}>{p.nome}</span>
      ...
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>
        ...
        return <tr key={p.id}>
          ...
          <td style={{...S.td,fontSize:11,color:'#64748B'}}>{p.categoria||'—'}</td>
          <td style={{...S.td,fontSize:11,color:'#64748B'}}>{p.unidade}</td>
          <td style={{...S.td,fontWeight:700,color:zero?'#EF4444':low?'#F59E0B':'#22C55E'}}>{p.quantidade}</td>
          <td style={{...S.td,color:'#94A3B8'}}>{p.estoque_minimo}</td>
          <td style={{...S.td,fontSize:11,color:'#64748B'}}>{p.localizacao||'—'}</td>
    ...
    <Modal open={!!movModal} onClose={()=>setMovModal(null)} title="MOVIMENTAÇÃO" mobile={vp.isMobile}>
      {movModal&&(()=>{const p=data.find(x=>x.id===movModal);if(!p)return null;return<div>
        <div style={{background:'#F1F5F9',padding:14,borderRadius:8,marginBottom:16}}>
          <div style={{fontSize:15,fontWeight:700}}>{p.nome}</div>
          <div style={{fontSize:13,color:'#64748B'}}>Cód: {p.codigo} · Estoque: <strong style={{color:ACCENT}}>{p.quantidade} {p.unidade}</strong></div>
        </div>
    ...
    <Modal open={importModal} onClose={()=>setImportModal(false)} title="📥 IMPORTAR ESTOQUE" mobile={vp.isMobile}>
      <div>
        <div style={{background:'#F1F5F9',borderRadius:8,padding:14,marginBottom:14,fontSize:12,color:'#334155',lineHeight:1.8}}>
          ...
          <div style={{marginTop:8,background:'#F8FAFC',borderRadius:6,padding:10,fontFamily:'monospace',fontSize:10,color:'#64748B'}}>
        ...
        {importStatus && <div style={{
          background:importStatus.includes('Concluído')?'#F0FDF4':importStatus.includes('erro')||importStatus.includes('não')?'#FEF2F2':'#DBEAFE',
          ...
          color:importStatus.includes('Concluído')?'#22C55E':importStatus.includes('erro')?'#EF4444':'#2563EB',
        }}>{importStatus}</div>}

        {importData.length > 0 && <>
          <div style={{fontSize:11,color:'#64748B',marginBottom:8}}>Pré-visualização ({importData.length} itens):</div>
          <div style={{maxHeight:200,overflow:'auto',background:'#F8FAFC',borderRadius:6,padding:8,marginBottom:14}}>
            <table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr>
              ...
              <tr key={i}>
                ...
                <td style={{...S.td,padding:'4px 8px',fontSize:10,color:'#64748B'}}>{r.unidade}</td>
                <td style={{...S.td,padding:'4px 8px',fontSize:10}}>{r.quantidade}</td>
                <td style={{...S.td,padding:'4px 8px',fontSize:10,color:'#64748B'}}>{r.estoque_minimo}</td>
              </tr>
            )}</tbody></table>
            {importData.length > 20 && <div style={{textAlign:'center',padding:6,color:'#94A3B8',fontSize:10}}>...e mais {importData.length-20} itens</div>}
          </div>
...
function PecaForm({ item, setItem, modal, onSave, onCancel, onDel, mobile }) {
  ...
  return <div>
    ...
    <div style={{marginTop:16,background:'#F1F5F9',borderRadius:8,padding:14}}>
      ...
      {loadingEq ? <div style={{color:'#94A3B8',fontSize:11,padding:10,textAlign:'center'}}>Carregando equipamentos...</div> :
        <div style={{maxHeight:200,overflow:'auto',border:'1px solid #222',borderRadius:6}}>
          {filteredEquips.slice(0,50).map(eq => {
            ...
            return <label key={eq.id} style={{
              ...
              <div style={{flex:1}}>
                <span style={{fontSize:12,color:checked?ACCENT:'#94A3B8',fontWeight:checked?600:400}}>{eq.nome}</span>
                {eq.codigo&&<span style={{fontSize:10,color:'#94A3B8',marginLeft:6}}>({eq.codigo})</span>}
              </div>
            </label>
          })}
          {filteredEquips.length > 50 && <div style={{padding:8,textAlign:'center',fontSize:10,color:'#94A3B8'}}>
            Mostrando 50 de {filteredEquips.length} — refine a busca
          </div>}
          {filteredEquips.length === 0 && <div style={{padding:12,textAlign:'center',fontSize:11,color:'#94A3B8'}}>
...
function MaterialHistorico({ mat }) {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const { data: osUso } = await supabase.from('os_materiais')
        .select('*, ordens_servico(numero_ordem,titulo,data_abertura,equipamento_id,equipamentos(nome,codigo),areas(nome),status_os(nome,icone))')
  ...
  return <div>
    {/* Info do material */}
    <div style={{ background: '#F1F5F9', borderRadius: 8, padding: 12, marginBottom: 14, borderLeft: `3px solid ${ACCENT}` }}>
      ...
      <div style={{ fontSize: 13, color: '#0F172A', marginTop: 6 }}>
        ...
        <span style={{ color: '#94A3B8' }}> / mín: {mat.estoque_minimo}</span>
      </div>
    </div>
    ...
    {data.equipFromOS.length > 0 && <div style={{ marginBottom: 14 }}>
      <div style={{ fontSize: 10, color: '#64748B', textTransform: 'uppercase', marginBottom: 8, fontWeight: 600 }}>⚙️ Máquinas que Utilizam ({data.equipFromOS.length})</div>
      {data.equipFromOS.map(eq => (
        <div key={eq.nome} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid #F1F5F9' }}>
          <div>
            <span style={{ fontSize: 12, color: '#0F172A', fontWeight: 600 }}>{eq.nome}</span>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <span style={{ fontSize: 10, color: '#94A3B8', marginLeft: 6 }}>({eq.qtdTotal} {mat.unidade})</span>
          </div>
        </div>
      ))}
    </div>}
    ...
    {data.equipVinc.length > 0 && <div style={{ marginBottom: 14 }}>
      <div style={{ fontSize: 10, color: '#64748B', textTransform: 'uppercase', marginBottom: 8, fontWeight: 600 }}>🔗 Vinculado a Equipamentos ({data.equipVinc.length})</div>
      {data.equipVinc.map(v => (
        <div key={v.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #F1F5F9', fontSize: 11 }}>
          <span style={{ color: '#334155' }}>{v.equipamentos?.nome} {v.equipamentos?.codigo ? `(${v.equipamentos.codigo})` : ''}</span>
          <span style={{ color: '#64748B' }}>Qtd padrão: {v.quantidade_padrao} · {v.equipamentos?.areas?.nome || ''}</span>
        </div>
      ))}
    </div>}
    ...
    {data.osUso.length > 0 && <div style={{ marginBottom: 14 }}>
      <div style={{ fontSize: 10, color: '#64748B', textTransform: 'uppercase', marginBottom: 8, fontWeight: 600 }}>📋 Uso em Ordens de Serviço ({data.osUso.length})</div>
      <div style={{ maxHeight: 220, overflow: 'auto' }}>
        {data.osUso.map(o => (
          <div key={o.id} style={{ padding: '5px 0', borderBottom: '1px solid #F1F5F9', fontSize: 11 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: '#0F172A', fontWeight: 600 }}><span style={{color:ACCENT}}>#{o.ordens_servico?.numero_ordem||'—'}</span> {(o.ordens_servico?.titulo || '').substring(0, 30)}</span>
    ...
    {data.movs.length > 0 && <div>
      <div style={{ fontSize: 10, color: '#64748B', textTransform: 'uppercase', marginBottom: 8, fontWeight: 600 }}>📊 Movimentações ({data.movs.length})</div>
      <div style={{ maxHeight: 200, overflow: 'auto' }}>
        {data.movs.map(m => (
          <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid #F1F5F9', fontSize: 11 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <span style={{ color: m.tipo === 'entrada' ? '#22C55E' : '#EF4444', fontWeight: 700 }}>{m.tipo === 'entrada' ? '↓' : '↑'} {m.quantidade}</span>
              <span style={{ color: '#64748B' }}>{m.observacoes || ''}</span>
            </div>
            <span style={{ color: '#94A3B8' }}>{fmtDate(m.created_at)}</span>
          </div>
        ))}
      </div>
    </div>}
    ...
    {data.movs.length === 0 && data.osUso.length === 0 && data.equipFromOS.length === 0 &&
      <div style={{ textAlign: 'center', padding: 20, color: '#94A3B8', fontSize: 12 }}>Nenhum histórico de uso registrado ainda</div>}
  </div>
}
