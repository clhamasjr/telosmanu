...
export default function Layout({ page, setPage, children, vp }) {
  ...
  return <div style={{ fontFamily:FONT,background:'#0A0A0B',color:'#E5E5E5',minHeight:'100vh',fontSize:13 }}>
    ...
    <aside style={{ width:W,background:'#111113',borderRight:'1px solid #1F1F23',display:'flex',flexDirection:'column',position:'fixed',top:0,left:0,bottom:0,zIndex:300,transform:vp.isDesktop?'none':open?'translateX(0)':`translateX(-${W+10}px)`,transition:'transform .3s cubic-bezier(.4,0,.2,1)',boxShadow:!vp.isDesktop&&open?'4px 0 24px rgba(0,0,0,.5)':'none' }}>
      ...
      <nav style={{ flex:1,padding:'8px 0',overflowY:'auto' }}>
        {NAV.map(n => <button key={n.key} onClick={()=>go(n.key)} style={{
          ...
          color:page===n.key?ACCENT:'#888',fontSize:12,fontFamily:FONT,cursor:'pointer',textAlign:'left',minHeight:42
    ...
    {vp.isMobile && <nav style={{ position:'fixed',bottom:0,left:0,right:0,height:60,background:'#111113',borderTop:'1px solid #1F1F23',display:'flex',zIndex:200,paddingBottom:'env(safe-area-inset-bottom, 0px)',overflowX:'auto' }}>
      {NAV.slice(0,6).map(n => <button key={n.key} onClick={()=>go(n.key)} style={{ flex:'1 0 auto',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:2,background:'none',border:'none',cursor:'pointer',color:page===n.key?ACCENT:'#666',fontSize:9,fontFamily:FONT,minWidth:50,padding:'0 4px' }}>
        <span style={{ fontSize:18 }}>{n.icon}</span><span>{n.short}</span>
      </button>)}
    </nav>}

    <main style={{ marginLeft:vp.isDesktop?W:0,padding:vp.isMobile?'68px 16px 76px':vp.isTablet?'68px 24px 24px':'24px 32px',minHeight:'100vh' }}>
      {children}
    </main>
  </div>
}
