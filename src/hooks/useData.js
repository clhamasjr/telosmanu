...
export function useOSPorMecanico(nome, periodo, mecanicoId) {
  const [data, setData] = useState([]); const [loading, setLoading] = useState(true)
  useEffect(() => {
    if (!nome) return
    setLoading(true)
    const from = (periodo?.from || '2020-01-01') + 'T00:00:00'
    const to = (periodo?.to || '2030-01-01') + 'T23:59:59'
    
    Promise.all([
      // 1) Buscar por executado_por (OS importadas SOFMAN)
      supabase.from('ordens_servico')
        .select('id,numero_ordem,titulo,data_abertura,data_conclusao,tempo_execucao_min,executado_por,status_os(nome,cor,icone),areas(nome),equipamentos(nome)')
        .ilike('executado_por', `%${nome}%`)
        .gte('data_abertura', from).lte('data_abertura', to)
        .order('data_abertura', { ascending: false }).limit(500),
      // 2) Buscar por os_mecanicos (OS novas com multi-mecânico)
      mecanicoId ? supabase.from('os_mecanicos')
        .select('ordem_servico_id,ordens_servico(id,numero_ordem,titulo,data_abertura,data_conclusao,tempo_execucao_min,executado_por,status_os(nome,cor,icone),areas(nome),equipamentos(nome))')
        .eq('mecanico_id', mecanicoId).limit(500) : Promise.resolve({ data: [] }),
    ]).then(([r1, r2]) => {
      const osMap = {}
      // Merge sem duplicatas
      ;(r1.data || []).forEach(o => { osMap[o.id] = o })
      ;(r2.data || []).forEach(om => {
        const o = om.ordens_servico
        if (o && !osMap[o.id]) {
          // Filtrar por período
          if (o.data_abertura && o.data_abertura >= from && o.data_abertura <= to) {
            osMap[o.id] = o
          }
        }
      })
      const merged = Object.values(osMap).sort((a, b) => (b.data_abertura || '').localeCompare(a.data_abertura || ''))
      setData(merged)
      setLoading(false)
    }).catch(() => setLoading(false))
  }, [nome, mecanicoId, periodo?.from, periodo?.to])
  return { data, loading }
}
